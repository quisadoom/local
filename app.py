...
def validate_and_load_data():
    ...
    try:
        # xlrd 엔진을 사용하여 구형 KCI 엑셀 형식을 안전하게 로드
...
def preprocess_data(df):
    ...
    if '발행연도' in df.columns:
        ...
        df = df.dropna(subset=['발행연도']) # 연도가 없는 논문은 추이에서 제외
...
def render_tab1(df):
    ...
    fig.add_trace(
        go.Bar(x=yearly['발행연도'], y=yearly['논문수'], name="발행된 논문 수", marker_color='rgba(55, 83, 109, 0.8)'),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(x=yearly['발행연도'], y=yearly['인용수'], name="총 피인용 수", mode='lines+markers', line=dict(color='#FF4B4B', width=3)),
        secondary_y=True,
    )
...
def render_tab4(df):
    ...
    # 테이블에 출력할 핵심 항목 필터링
    display_cols = [c for c in ['발행연도', '논문명', '저자명', '학술지명', '인용된 총 횟수', '저자키워드'] if c in df.columns]
    
    st.write("아래 리스트는 **발행연도 최신순 > 인용된 총 횟수 랭킹순**으로 정렬되어 있습니다. (테이블 헤더를 클릭하면 정렬 방향을 바꿀 수 있습니다)")
        ...
        if search_kw:
            ...
            if matched.empty:
                ...
                for idx, row in matched.iterrows():
                    with st.expander(f"📖 {row['논문명']} - {row['저자명']} (인용 수: {row['인용된 총 횟수']})"):
                        abstract_text = row['초록'] if pd.notna(row['초록']) else "초록 정보가 기재되지 않은 문헌입니다."
                        st.markdown(f"**학술지:** {row.get('학술지명', '미상')} ({row['발행연도']})")
